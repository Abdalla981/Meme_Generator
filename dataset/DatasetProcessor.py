...
class DatasetProcessor(Dataset):
    def __init__(self, captions_path, images_path, start_token='startseq', end_token='endseq',
                 oov_token='[UNK]', split_value=(0.8, 0.1)):
    ...
    def create_sequences(self, captions, image):
        X1, X2, y = [], [], []
    ...
    def data_generator(self, captions_dict, images_dict):
        while 1:
            names = list(captions_dict.keys())
            np.random.shuffle(names)
            for name in names:
                captions = captions_dict[name]
                image = np.zeros((300)) # will be rewritten to fetch image feature
    ...
    def print_sample_of_sequences(self, captions_dict=None, images_dict=None, num=2):
        if captions_dict is None:
        ...
        print('{0:<10}'.format('Image size'), end=' | ')
        print('{0:<100}'.format('Sequences'), end=' | ')
        print('{0:<15}'.format('Out word'))
        for i in range(num):
            out_word = ''
            print('{0:<10}'.format(f'Caption {i}'))
            while out_word != self.end_token:
                print('{0:<10}'.format(f'{inputs[0][j].shape}'), end=' | ')
